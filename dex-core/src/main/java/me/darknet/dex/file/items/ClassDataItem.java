...
public record ClassDataItem(List<EncodedField> staticFields, List<EncodedField> instanceFields,
    ...
    public static final ItemCodec<ClassDataItem> CODEC = new ItemCodec<>() {
        ...
        public ClassDataItem read0(Input input, DexMapAccess context) throws IOException {
            int staticFieldsSize = (int) input.readULeb128();
            int instanceFieldsSize = (int) input.readULeb128();
            int directMethodsSize = (int) input.readULeb128();
            int virtualMethodsSize = (int) input.readULeb128();
            ...
            for (int i = 0; i < staticFieldsSize; i++) {
                int fieldIndexDiff = (int) input.readULeb128();
                int accessFlags = (int) input.readULeb128();
            ...
            for (int i = 0; i < directMethodsSize; i++) {
                int methodIndexDiff = (int) input.readULeb128();
                int accessFlags = (int) input.readULeb128();
                int codeOffset = (int) input.readULeb128();
